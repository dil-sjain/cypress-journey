...
class addToCartPage {
    ...
    verifyCartCount(initialCount, expected_cart_count){
      this.getCartCount().then((initialCount) => {
    ...
    verifyProductTitleFromCart(product_name){
      performAssert.elementVisibility(this.webLocators.cartSectionTitle());
      performAssert.elementContainText(this.webLocators.productTitle(), product_name);
    }
    removeItemsFromCart(){
    ...
    verifyCheckOutPageUrl(expected_checkout_url){
    ...
    verifyCustomeNameOnDeliveryPage(name){
      performAssert.elementTextWithTrim(this.getDeliveringCustomerName(), name);
        performAssert.elementTextWithTrim(this.getDeliveryAddress(), expected_checkout_shipping_address);
    }
    verifyAddressOnDeliveryPage(expected_checkout_shipping_address){
      performAssert.elementTextWithTrim(this.getDeliveryAddress(), expected_checkout_shipping_address);
    }

    verifyProductPriceOnDeliveryPage(expected_product_price){
      this.webLocators.orderPrice().invoke('text').then((text) => {
            const price = parseFloat(text.replace(/[^0-9.]/g, ''));
            performAssert.expectedValue(price, expected_product_price);
        });
    ...
    verifySaveForLater(product_name){
      performAssert.elementVisibility(this.webLocators.saveForLaterSection());
      performAssert.elementContainText(this.webLocators.saveForLaterProduct(),product_name)
    }
    ...
    verifyRemovedText(){
      performAssert.elementVisibility(this.webLocators.removedText());
    }
}
